...
#define BUF_SIZE 256
static unsigned int PNM_MAX = 65535;
...
static image<uchar> *loadPBM(const char *name) {               //unused
  char buf[BUF_SIZE];
...
static void savePBM(image<uchar> *im, const char *name) {            //unused
  int width = im->width();
...
static image<uchar> *loadPGM(const char *name) {              //unused
  char buf[BUF_SIZE];
...
static void savePGM(image<uchar> *im, const char *name) {            //unused
  int width = im->width();
  int height = im->height();
  std::ofstream file(name, std::ios::out | std::ios::binary);
...
static void saveEXCEL(image<ushort> *im, const char *name) {
    int width = im->width();
    int height = im->height();
